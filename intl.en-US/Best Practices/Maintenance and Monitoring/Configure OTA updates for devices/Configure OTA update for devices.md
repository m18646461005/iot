...
You have created a product and devices in the IoT Platform console and obtained the device certificate information.

In this example, the device development uses the Link Kit SDK for C provided by Alibaba Cloud. You can visit [Link Kit SDK](https://www.alibabacloud.com/help/product/93051.htm) to download the SDK demo.

In the SDK demo, fota indicates OTA update.

## Connect devices to IoT Platform

1.  Use the make menuconfig command to enter the compilation settings. In compilation settings, select FEATURE\_OTA\_ENABLED \(NEW\) to enable OTA updates for the device.

    ![IoT Platform firmware update](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/en-US/9937549951/p66021.png)

2.  Go to the src/dev\_model/examples directory and open the Linkkit\_example\_solo.c sample code.

    The sample file contains the following code for firmware update.
...
3.  Edit the device information in the linkkit\_example\_solo.c file.
...
4.  Run the program and connect the device to the IoT Platform.


## Update firmware configurations

1.  You can edit the Linkkit\_example\_solo.c file to update the configurations based on your workload requirements.

    For more information about the updated files in this example, see the appendix of this topic.

2.  Compile the updated Linkkit\_example\_solo.c file and save the compiled BIN file.

    In this example, the saved BIN file is named app\_1001.

    When you proceed to [Push firmware to devices](/intl.en-US/Best Practices/Maintenance and Monitoring/Configure OTA updates for devices/Push firmware to devices.md), you must create a new firmware in the IoT Platform console and upload the BIN file.
